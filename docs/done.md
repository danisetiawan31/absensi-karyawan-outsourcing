...
## [Stage 44] Track J — Leave Requests Mobile (leave-requests-karyawan)

- **Selesai:** Implementasi penuh fitur pengajuan & riwayat izin karyawan pada mobile app (`src/app/(karyawan)/izin.tsx` & `src/app/(karyawan)/leave-request-create.tsx`), service layer `leave-requests.service.ts`, penambahan 4 komponen UI reusable terpusat (`ScreenHeader`, `AsyncStateViews`, `AlertBanner`, `ConfirmModal`), serta penyesuaian hex color ke token semantik `theme.ts` (`COLORS`).
- **File Dibuat/Diubah:** `services/leave-requests.service.ts` _(NEW)_, `services/__tests__/leave-requests.service.test.ts` _(NEW)_, `types/leave-request.ts` _(NEW)_, `constants/theme.ts`, `components/` (`ScreenHeader.tsx` _(NEW)_, `AsyncStateViews.tsx` _(NEW)_, `AlertBanner.tsx` _(NEW)_, `ConfirmModal.tsx` _(NEW)_, `StatusBadge.tsx`, `SectionCard.tsx`), `components/__tests__/` (`ScreenHeader.test.tsx` _(NEW)_, `AsyncStateViews.test.tsx` _(NEW)_, `AlertBanner.test.tsx` _(NEW)_, `ConfirmModal.test.tsx` _(NEW)`), `screens/karyawan/` (`IzinScreen.tsx`_(NEW)_,`LeaveRequestCreateScreen.tsx` _(NEW)`), `screens/karyawan/__tests__/` (`IzinScreen.test.tsx` _(NEW)_, `LeaveRequestCreateScreen.test.tsx` _(NEW)`), `app/(karyawan)/` (`izin.tsx`, `leave-request-create.tsx` _(NEW)`), `docs/feature/leave-requests-karyawan.md` _(NEW)_.
- **Verifikasi:** Full mobile test suite (19/19 test suites, 129/129 tests pass), `npx tsc --noEmit` PASS (0 error, ZERO `any`).
- **Catatan Penting:**
  - **4 Komponen Reusable Baru & Refactoring Token**: Pembangunan 4 komponen UI terpusat (`ScreenHeader`, `AsyncStateViews`, `AlertBanner`, `ConfirmModal`) yang diekstrak & diretrofit dari `IzinScreen.tsx` dan `LeaveRequestCreateScreen.tsx`. Seluruh kelas warna mentah (`rose-*`, `emerald-*`, `amber-*`, hex inline) diselaraskan ke token semantik resmi project (`primary`, `success`, `warning`, `destructive`, `muted`, `slate400`).
  - **Double-Tap Guard & Direct Reactive Gate**: Submit form di-guard menggunakan `useRef` lock (`isSubmittingRef`) secara sinkron untuk mencegah race condition. Validasi client-side `DOKUMEN_WAJIB` bekerja secara reaktif (`jenis === 'SAKIT'` DAN `tanggalSelesai > tanggalMulai` strictly lebih besar) serta menolak file >5MB sebelum upload HTTP.
  - **Refetch & Race Handling (409)**: Pembatalan izin (`PATCH /leave-requests/:id/cancel`) hanya untuk status `PENDING`. Skenario 409 (`IZIN_SUDAH_DIPROSES`) di-handle dengan refetch otomatis dan pesan informatif tanpa treat sebagai error generik. Navigasi balik otomatis me-trigger TanStack Query cache invalidation `['leave-requests']`.
  - **Interoperabilitas Timezone Backend-Mobile**: Mobile mengirimkan string tanggal murni `YYYY-MM-DD`, backend secara otomatis menafsirkan dan menguncinya ke `00:00:00+07:00` (Asia/Jakarta) via `getJakartaStartOfDay()`.

## [Stage 45] Track J — Notifikasi Mobile Karyawan (notifikasi-mobile)

- **Selesai:** Implementasi penuh fitur riwayat notifikasi & tandai dibaca untuk karyawan (`src/app/(karyawan)/notifikasi.tsx` & `src/screens/karyawan/NotifikasiScreen.tsx`), service layer `notifications.service.ts`, tipe data `notification.ts`, utilitas tanggal terpusat `src/utils/date.util.ts`, serta unit testing komprehensif.
- **File Dibuat/Diubah:** `services/notifications.service.ts` _(NEW)_, `services/__tests__/notifications.service.test.ts` _(NEW)_, `types/notification.ts` _(NEW)_, `utils/date.util.ts` _(NEW)_, `utils/__tests__/date.util.test.ts` _(NEW)_, `screens/karyawan/NotifikasiScreen.tsx` _(NEW)_, `screens/karyawan/__tests__/NotifikasiScreen.test.tsx` _(NEW)_, `screens/karyawan/IzinScreen.tsx`, `screens/karyawan/LeaveRequestCreateScreen.tsx`, `screens/karyawan/__tests__/LeaveRequestCreateScreen.test.tsx`, `app/(karyawan)/notifikasi.tsx`, `docs/feature/notifikasi-mobile.md` _(NEW)_, `docs/backlog.md`, `docs/done.md`.
- **Verifikasi:** Full mobile test suite (22/22 test suites, 148/148 tests pass), `npx tsc --noEmit` PASS (0 error, ZERO `any`).
- **Catatan Penting:**
  - **Pemetaan 4 Tipe Notifikasi & Graceful Fallback**: Mendukung pemetaan visual `REMINDER_CHECKIN` (aksen warning) dan `PERUBAHAN_JADWAL` (aksen info). Tipe `ALERT_SUPERVISOR` / `PENGAJUAN_IZIN_ORPHANED` atau tipe tidak dikenal di-handle secara graceful menggunakan varian `muted` dan ikon default tanpa crash.
  - **Optimistic Update, Double-Tap Lock (Set Ref), & Revert on Error**: Saat item unread ditap, cache query `['notifications']` secara instan diperbarui ke `dibaca: true`. Tap ganda sinkron pada item yang sama di-guard via `pendingMarkAsReadIdsRef` (`Set<string>`). Jika HTTP request `PATCH /notifications/:id/read` gagal, state cache di-revert ke `dibaca: false` dan `<AlertBanner type="error">` non-blocking ditampilkan. Tap pada item yang sudah dibaca secara bijak di-skip dari request HTTP tambahan.
